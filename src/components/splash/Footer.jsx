...
export default function Footer({ fadeOut, fadeIn }) {
    return (
        <>
            <div className={`absolute bottom-0 left-0 right-0 h-20 sm:h-24 bg-gradient-to-t from-white/50 to-transparent backdrop-blur-sm border-t border-white/20 transition-opacity duration-800 ${fadeOut ? 'opacity-0' : 'opacity-100'
                }`} />

            <div className={`absolute bottom-6 sm:bottom-8 left-1/2 transform -translate-x-1/2 text-center transition-all duration-1000 ${!fadeIn ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'
